...
def multi_layer_feature(body, from_layers, num_filters, strides, pads, min_filter=128):
    ...
    if not isinstance(body,list):
        ...
        layers = []
        for k, params in enumerate(zip(from_layers, num_filters, strides, pads,body)):
            from_layer, num_filter, s, p,layer = params
            # num_1x1 = max(min_filter, num_filter // 2)
            # conv_1x1 = conv_act_layer(layer, 'multi_feat_%d_conv_1x1' % (k),
            #
            #                          num_1x1, kernel=(1, 1), pad=(0, 0), stride=(1, 1), act_type='relu')
            # num_filter=512
            num_filter=256
            # conv_3x3 = mx.symbol.Activation(data=layer, act_type='relu')
            # if private_config.enable_aspp:
            #     aspp_list = list()
            #     for i in range(4):
            #         pad = ((i + 1) * 4, (i + 1) * 4)
            #         dilate = pad
            #         conv_aspp = mx.symbol.Convolution(data=conv_3x3, num_filter=256, kernel=(3, 3), pad=pad,
            #                                           dilate=dilate, name=('fc1_aspp_%s_c%d' % (k, i)))
            #         aspp_list.append(conv_aspp)

            #     conv_3x3 = mx.symbol.ElementWiseSum(*aspp_list, name=('fc1_aspp_%s' % k))
            #     conv_3x3 = mx.symbol.Activation(data=conv_3x3, act_type='relu')
            # for i in range(2):
            #     conv_3x3 = conv_act_layer(conv_3x3, 'multi_feat_%d_conv_3x3_%d' % (k,i),
            #                               num_filter, kernel=(3, 3), pad=(1, 1), stride=(1, 1), act_type='relu')
            # conv_3x3 = conv_act_layer(layer, 'multi_feat_%d_conv_3x3' % (k),
            #                           num_filter, kernel=(1, 1), pad=(0, 0), stride=(1, 1), act_type='relu',use_batchnorm=True)
            # conv_3x3 = conv_act_layer(layer, 'multi_feat_%d_conv_3x3' % (k),
            #                           num_filter, kernel=(3, 3), pad=(1, 1), stride=(1, 1), act_type='relu')
            # layers.append(conv_3x3)
            layers.append(layer)
...
def multibox_layer(from_layers, num_classes, sizes=[.2, .95],
    ...
    for k, from_layer in enumerate(from_layers):
        ...
        bias = mx.symbol.Variable(name="{}_binary_cls_pred_conv_bias".format(from_name),
            init=mx.init.Constant(0.0), attr={'__lr_mult__': '2.0'})
        # cls_pred = mx.symbol.Convolution(data=from_layer, bias=bias, kernel=(3,3), \
        #     stride=(1,1), pad=(1,1), num_filter=num_cls_pred, \
        #     name="{}_binary_cls_pred_conv".format(from_name))
        cls_pred = mx.symbol.Convolution(data=from_layer, bias=bias, kernel=(1,1), \
            stride=(1,1), pad=(0,0), num_filter=num_cls_pred, \
            name="{}_binary_cls_pred_conv".format(from_name))
        cls_pred = mx.symbol.transpose(cls_pred, axes=(0,2,3,1))
        cls_pred = mx.symbol.Flatten(data=cls_pred)
        binary_cls_pred_layers.append(cls_pred)
    ...
    binary_cls_preds = mx.symbol.Concat(*binary_cls_pred_layers, num_args=len(binary_cls_pred_layers), \
        dim=1)
    binary_cls_preds = mx.symbol.Reshape(data=binary_cls_preds, shape=(0, -1, 2))
    binary_cls_preds = mx.symbol.transpose(binary_cls_preds, axes=(0, 2, 1), name="multibox_binary__cls_pred")
    ...
    return [loc_preds, cls_preds,binary_cls_preds, anchor_boxes]
